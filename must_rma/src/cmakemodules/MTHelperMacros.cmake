...
####################################################
## Macro addTsanTestExecutable
##
## Creates an executable target for a test MPI application using
## the PnMPI library as dependency.
####################################################
MACRO (addTSanTestExecutable targetName sourceList)
    #Set libs, dirs, ... for executable
    SET(CMAKE_C_FLAGS ${MPI_C_COMPILE_FLAGS})
    SET(CMAKE_CXX_FLAGS ${MPI_CXX_COMPILE_FLAGS})
    SET(CMAKE_Fortran_FLAGS ${MPI_Fortran_COMPILE_FLAGS})
    ...
    #Add test case
    ADD_EXECUTABLE (${targetName} ${sourceList})
    # Compile test cases with -O0 (to avoid false negatives due to compiler optimizations) and TSan instrumentation
    TARGET_COMPILE_OPTIONS(${targetName} PRIVATE "-O0" "-gdwarf-2" "-fno-omit-frame-pointer" "-fsanitize=thread")
    TARGET_LINK_LIBRARIES(${targetName} PRIVATE "-fsanitize=thread" ${TEST_LIBRARIES})
ENDMACRO (addTSanTestExecutable)
