  ...
  myLbInfo (NULL)
{
    ...
    {
        must::cerr << "Error: the CommTrack module needs the GroupTrack module as a child, but it was not specified." << std::endl;
        assert (0);
    }
...
DatatypeTrack::~DatatypeTrack ()
{
    ...
    if (CacheHitCount+CacheMissCount>0)
        must::cout << "Cache matches: " << CacheHitCount << " misses: " << CacheMissCount << std::endl;
#endif
}
...
// contents
//=============================
GTI_ANALYSIS_RETURN DatatypeTrack::contents (
        MustParallelId pId,
        MustLocationId lId,
        MustDatatypeType type
)
...
)
{
    ...
    FullDupTypeInfo *info = new FullDupTypeInfo (
            this,
            pId,
            lId,
            oldInfos,
...
std::string DatatypeTrack::getPredefinedName (MustMpiDatatypePredefined predefined)
{
    switch (predefined)
    {
        ...
        case MUST_MPI_AINT: return "MPI_AINT"; break;
        case MUST_MPI_OFFSET: return "MPI_OFFSET"; break;
        case MUST_MPI_COUNT: return "MPI_COUNT"; break;
    ...
    else
    {
        switch ((MustMpiDatatypePredefined) predef)
        {
            ...
            case MUST_MPI_DATATYPE_UNKNOWN:
                ...
                break;
/*            default:
                    must::cerr << "Controll completness of base types in " << __FILE__ << "@" << __LINE__ << " missing predef has number " << predef << std::endl;
                    assert (0);*/
    ...
    //{
    //    must::cout << "Predef: " << getPredefinedName (iter2->second)  << " value " << iter2->first << std::endl;
    //}
}
...
// 	std::map <MustDatatypeType, Datatype*>::iterator iter;
	...
	for (int i = 0; i < numPredefs; i++)
	{
		...
		{
#ifdef MUST_DEBUG
			must::cerr<<"Internal warning in " << __FILE__ << "@" << __LINE__ << " for mpi type (must enum) " << predefinedIds[i] << ", there is no full info for this type, likely due to two MPI datatypes havin the same handle (e.g. MPI_LONG_LONG and MPI_LONG_LONG_INT in OpenMPI)."<< std::endl;
#endif
			continue;
		}

		((FullBaseTypeInfo*)iter->second)->setSizes(extents[i], alignments[i]);
// 		must::cout << "DT Info, " << iter->second->getPredefinedName() << " extent=" << iter->second->isNull() << " alignment=" << iter->second->getAlignment() << std::endl;
	}
        ...
        MustRemoteIdType baseType)
{
    ...
    //Create the resource
    FullDupTypeInfo* resource = new FullDupTypeInfo (
            this,
            creationPId,
            creationLId,
            baseTypeInfo,
        ...
        MustRemoteIdType* baseTypes)
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        if (!baseTypeInfo[i])
        {
            must::cerr << "Internal error in: " << __FILE__ << ":" << __LINE__ << " for count=" << i << std::endl;
            assert (0);
            return GTI_ANALYSIS_FAILURE;
        }
        else
