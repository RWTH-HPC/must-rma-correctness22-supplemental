...
#endif
	if (retValue != MPI_SUCCESS) must::cerr << "WARNING: MPI_Type_extent(" << mpiId << ") failed" << std::endl;
...
WRAP_MPI_CALL_POSTFIX
	...
	{
		////must::cout << "ERROR: failed to get \"propagatePredefinedDatatypes\" function pointer from wrapper. Did you forgot to load the API group MUST_Resource_API ? Aborting now." << std::endl;
