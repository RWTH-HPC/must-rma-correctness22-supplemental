...
CommTrack::~CommTrack ()
{
	...
	for (std::vector<I_Module*>::size_type i = 0; i < mySelfCommInfos.size(); i++)
	{
	    ...
	    {
#ifdef MUST_DEBUG
            must::cout << "Warning: A MPI_COMM_SELF handle was queried for in a persistent way and was not freed." << std::endl;
#endif
	    }
	}
	...
	for (std::vector<I_Module*>::size_type i = 0; i < myWorldCommInfos.size(); i++)
	{
	    ...
	    {
#ifdef MUST_DEBUG
            must::cout << "Warning: A MPI_COMM_WORLD handle was queried for in a persistent way and was not freed." << std::endl;
#endif
	    }
	}
		...
		int *newRank2WorldArray)
{
	...
	//==2) Abort if new comm is null or a predefined value
        if (newcomm == myNullValue ||
            slock_safe_ptr(myPredefineds)->find(newcomm) !=
                slock_safe_ptr(myPredefineds)->end())
          return GTI_ANALYSIS_SUCCESS;
		...
		int contextId)
{
	...
	{
          auto s_safe_Predefineds = slock_safe_ptr(myPredefineds);
          if (s_safe_Predefineds->find(newintercomm) !=
                  s_safe_Predefineds->end() ||
		...
		int *newRank2WorldArray)
{
	...
	{
          auto s_safe_Predefineds = slock_safe_ptr(myPredefineds);
          if (s_safe_Predefineds->find(newintracomm) !=
                  s_safe_Predefineds->end() ||
...
{
	////DEBUG
	//must::cout << "CommTrack: " << reachableBegin << "-" << reachableEnd << " size=" << worldSize;
	//if (channId)
	//	must::cout << " id=" << channId->toString();
	//must::cout << std::endl;
	////END DEBUG
	...
	{
		if (iter != myCommWorldHandles.begin()) must::cout << ", ";
		...
		MustCommType comm)
{
	...
	//Is it a user defined type ?
        ...
        if (pos == slock_safe_ptr(myUserHandles)->end()) {
          ...
          std::map<int, MustCommType>::iterator wHI =
              myCommWorldHandles.find(rank);
          if (wHI != myCommWorldHandles.end() &&
              (wHI->second == comm || myRealCommWorld == comm)) {
