	...
	: TrackBase<Request, I_Request, MustRequestType, MustMpiRequestPredefined, RequestTrack, I_RequestTrack> (instanceName)
{
    ...
    {
        must::cerr << "Error: the RequestTrack module needs the DatatypeTrack and CommTrack modules as childs, but at least one of them was not available." << std::endl;
        assert (0);
    }
		...
		int numIndices)
{
	...
	for (int i = 0; i < numIndices; i++)
	{
		if (indices[i] >= count)
		{
			must::cerr << "Error: an index in completeSome (" << __FILE__<< "@" << __LINE__ << ") is larger outside the array of indices (indices[" << i << "]=" << indices[i] << ", count=" << count << "). This is an error in the MPI or MUST implementation." << std::endl;
			return GTI_ANALYSIS_FAILURE;
		}
