	...
	for (i = myGroupTables.begin(); i != myGroupTables.end(); i++)
	{
		...
		for (j = i->second.begin(); j != i->second.end(); j++)
		{
			...
			for (k = j->second.begin(); k != j->second.end(); k++)
			{
				...
				must::cerr << "A GroupTable was not freed during shutdown, this suggest a reference count problem, but may also result from a non-optimal destruction order! Group size: " << group->getSize() << std::endl;
#endif
...
void GroupTrack::deleteGroupTable (I_GroupTable* group)
{
	...
	{
		//Internal error
		//must::cerr << "Internal Error: tried to free a group that does not exists. (" << __FILE__ << "@" << __LINE__ << ")" << std::endl;
		//assert (0);
        ...
        MustRemoteIdType remoteId)
{
    ...
    if (pos == myRemoteTables.end())
    {
        must::cerr << "Error: received a freeRemoteGroupTable for an unknown group table!" << std::endl;
        assert (0);
        return GTI_ANALYSIS_SUCCESS;
    }
...
void GroupTrack::printCollection (void)
{
		...
		for (j = i->second.begin(); j != i->second.end(); j++)
		{
			...
			for (k = j->second.begin(); k != j->second.end(); k++)
			{
				...
				if (group)
				{
					...
					count --;
					group->erase();
					must::cout << "[" << i->first << "]->[" << j->first.first << "," << j->first.second << "]->group with size=" << group->getSize() << " pointer=" << (unsigned long)group << " count=" << count << std::endl;
				}
				else
				{
	...
	//The current list of handles
        ...
        for (iter = s_safe_UserHandles->begin();
             iter != s_safe_UserHandles->end(); iter++) {
          must::cout << "Handle " << iter->first.second << "@"
                    << iter->first.first
                    << " groupPointer=" << (unsigned long)iter->second->myGroup
                    << std::endl;
          int count =
              (int)
                  iter->second->myGroup->copy(); // TODO this should return the
