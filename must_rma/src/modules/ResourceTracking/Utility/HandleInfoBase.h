...
namespace must
{
    ...
    protected:
        std::atomic<int> userRefCount; /**< The reference count used to determine the live span of this handle information, count of external (no MPI) uses of the handle.*/
        std::atomic<int> mpiRefCount; /**< The reference count used to determine the live span of this handle information, count of MPI use of this handle (Rational: MPI may return the same handle multiple times, i.e. MPI_Comm_group for OpenMPI).*/
        ...
        static std::atomic<bool> ourAllowFreeForwarding; /**< Global flag that specifies whether we should call the myPassFreeAcross function, reasons for it to be set to false include a starting shutdown of the place.*/
