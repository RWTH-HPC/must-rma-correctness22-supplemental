...
BaseConstants::BaseConstants(const char *instanceName)
  ...
  if (subModInstances.size() > 0) {
    for (std::vector<I_Module *>::size_type i = 0; i < subModInstances.size();
         i++)
...
PMPI_Comm_rank(MPI_COMM_WORLD, &rank);
         ...
         std::cout << rank << ": BaseConstants::isAnyTag : " << mpiAnyTag << "  myMpiAnyTag "  << myMpiAnyTag<< std::endl;
...
PMPI_Comm_rank(MPI_COMM_WORLD, &rank);
         ...
         std::cout << rank << ": BaseConstants::getAnyTag : " << myMpiAnyTag << std::endl;
	 return myMpiAnyTag;
}
