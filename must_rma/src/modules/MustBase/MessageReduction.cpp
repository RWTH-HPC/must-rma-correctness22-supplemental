...
{
    if (!hasLocation)
        return reduceStrided (msgId, pId, lId, fileId, 0, 0, 0, msgType, text, textLen, numReferences, refPIds, refLIds, thisChannel, outFinishedChannels);

    return reduceStrided (msgId, pId, lId, fileId, myPIdModule->getInfoForId(pId).rank, 1, 1, msgType, text, textLen, numReferences, refPIds, refLIds, thisChannel, outFinishedChannels);
}
...
)
{
    ...
    for (iter = myReps.begin(); iter != myReps.end(); iter++)
    {
        if (iter->belongsToRepresentation(msgId, fileId, callname, msgType, text, startRank, stride, count))
        {
...
: myMsgId (other.myMsgId),
  ...
  myAnalyser (other.myAnalyser),
  myMatchIndex (other.myMatchIndex),
  myMatch (other.myMatch)
{
    //Copy references arrays
                ...
                )
{
	...
	{
		if (myMatchIndex != -1)
		{ //use saved match to compute more efficient
			return myAnalyser->checkEquals(myMsgId, myMatchIndex, myMatch, msgId, text);
		}
		else
		{ //parsing failed first time. should mean: only callback registered, so no regex available
