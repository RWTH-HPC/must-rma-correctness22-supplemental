...
MessageFileReduction::MessageFileReduction(const char *instanceName)
    : ModuleBase<MessageFileReduction, I_MessageFileReduction>(instanceName),
      reduceCompletion(NULL),
      reducePartners(),
      reduceTimedOut(false),
      filterId(0)
{
    // create sub modules
...
#define NUM_MODS_REQUIRED 1
    ...
    if (subModInstances.size() > NUM_MODS_REQUIRED)
        for (vector<I_Module *>::size_type i = NUM_MODS_REQUIRED;
             i < subModInstances.size();
             i++)
 ...
 */
GTI_ANALYSIS_RETURN
MessageFileReduction::changeMessageFile(size_t fileId,
                                        const char *filename,
                                        size_t len)
{
 ...
 */
GTI_ANALYSIS_RETURN
MessageFileReduction::filter(size_t fileId,
                             const char *filename,
                             size_t len,
 ...
 */
GTI_ANALYSIS_RETURN
MessageFileReduction::reduce(size_t fileId,
                             I_ChannelId *thisChannel,
                             list<I_ChannelId *> *outFinishedChannels)
{
    ...
    if (!reduceCompletion)
        reduceCompletion =
            new CompletionTree(thisChannel->getNumUsedSubIds() - 1,
                               thisChannel->getSubIdNumChannels(
