...
namespace must
{
    ...
    public:
         ...
         */
                const LocationInfo &getInfoForId(MustParallelId pId,
                                                 MustLocationId lId);
    ...
    protected:
    		...
    		sf::contfree_safe_ptr<GlobalMapType> myGlobalMap; /**< Used if no local mapping of a location id is present.*/
    		sf::contfree_safe_ptr<LocalMapType> myLocalMap; /**< Mapping for an individual parallel element and an location id to the location information (local mapping).*/
