...
namespace must
{
    ...
    protected:
       ...
       * an occurrence count*/
      typedef std::map<
          int, std::pair<std::map<LocationInfoImpl<uint64_t>, MustLocationId>,
                         uint32_t>>
          KnownLocationsType;
#else
      /** Maps a GTI call id to a map of call stacks known for that call id and
       * an occurrence count*/
      typedef std::map<
          int, std::pair<std::map<LocationInfo, MustLocationId>, uint32_t>>
          KnownLocationsType;
#endif
#endif
#if defined(BUILD_BACKWARD)
         ...
         */
        void createHandleNewLocationCall (MustLocationId id, const char* callName, LocationInfo &location);
#endif
     ...
     */
    gti::GTI_ANALYSIS_RETURN initCodePtr(const char* callname,
