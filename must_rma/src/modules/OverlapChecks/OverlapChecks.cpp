...
//         tempList.sort();
    ...
    {
//         must::cout << "selfoverlapping buffer on " << iter->repetition << "(th) repetition at ";
//         typeinfo->printDatatypeLongPos(must::cout, iter->pos + iter->first - nextIter->first);
//         must::cout << " with " << nextIter->repetition  << "(th) repetition at ";
//         typeinfo->printDatatypeLongPos(must::cout, nextIter->pos);
//         must::cout << std::endl;
        return true;
    }
    return false;
        ...
        )
{
    if (sendcountslen<1 || sendtypeslen<1 || recvcountslen<1 || recvtypeslen<1 || senddisplslen<0 || recvdisplslen<0 || (senddisplslen>0 && senddispls==NULL) || (recvdisplslen>0 && recvdispls==NULL))
    {
        ...
        must::cout << "Implementation error: Malicious call of OverlapChecks::isSendRecvOverlappedN !" << std::endl;
    ...
    if (typeinfo == NULL)
    {
//        must::cout << "Datatype " << type << "not found in isSendRecvOverlappedN" << std::endl;
        return GTI_ANALYSIS_SUCCESS;
    }
    MustAddressType extent = typeinfo->getExtent();
