...
#define get_must_callback(name)                     \
    getWrapperFunction("MUST_MPIADT_callback_" #name "_pre", \
                       (GTI_Fct_t*) &callback_##name##_pre); \
    getWrapperFunction("MUST_MPIADT_callback_" #name "_post", \
                       (GTI_Fct_t*) &callback_##name##_post)
 ...
 * @param codeptr
 */
static void on_MPIADT_callback_Allgather(const void *sendbuf, int sendcount,
                                  ...
                                  void **tool_data, const void *codeptr)
{
    ...
    if (endpoint == MPIADT_endpoint_begin)
    {
        if (a->callback_Allgather_pre)
            a->callback_Allgather_pre(a->getParallelId(),a->getLocationId(codeptr),
            sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype, comm);
    } else {
        if (a->callback_Allgather_post)
            a->callback_Allgather_post(a->getParallelId(),a->getLocationId(codeptr),
            sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype, comm);
    }
}
 ...
 * @param codeptr
 */
static void on_MPIADT_callback_Allreduce(const void *sendbuf, void *recvbuf, int count,
                                  ...
                                  void **tool_data, const void *codeptr)
{
    ...
    if (endpoint == MPIADT_endpoint_begin)
    {
        if (a->callback_Allreduce_pre)
            a->callback_Allreduce_pre(a->getParallelId(),a->getLocationId(codeptr),
            sendbuf, recvbuf, count, datatype, op, comm);
    } else {
        if (a->callback_Allreduce_post)
            a->callback_Allreduce_post(a->getParallelId(),a->getLocationId(codeptr),
            sendbuf, recvbuf, count, datatype, op, comm);
    }
}
 ...
 * @param codeptr
 */
static void on_MPIADT_callback_Alltoall(const void *sendbuf, int sendcount,
                                 MPI_Datatype sendtype, void *recvbuf,
                                 int recvcount, MPI_Datatype recvtype,
                                 MPI_Comm comm, MPIADT_endpoint_t endpoint,
                                 void **tool_data, const void *codeptr)
{
    ...
    if (endpoint == MPIADT_endpoint_begin)
    {
        if (a->callback_Alltoall_pre)
            a->callback_Alltoall_pre(a->getParallelId(),a->getLocationId(codeptr),
            sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype, comm);
    } else {
        if (a->callback_Alltoall_post)
            a->callback_Alltoall_post(a->getParallelId(),a->getLocationId(codeptr),
            sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype, comm);
    }
}
 ...
 * @param codeptr
 */
static void on_MPIADT_callback_Gather(const void *sendbuf, int sendcnt,
                               MPI_Datatype sendtype, void *recvbuf,
                               ...
                               void **tool_data, const void *codeptr)
{
    ...
    if (endpoint == MPIADT_endpoint_begin)
    {
        if (a->callback_Gather_pre)
            a->callback_Gather_pre(a->getParallelId(),a->getLocationId(codeptr),
            sendbuf, sendcnt, sendtype, recvbuf, recvcnt, recvtype, root, comm);
    } else {
        if (a->callback_Gather_post)
            a->callback_Gather_post(a->getParallelId(),a->getLocationId(codeptr),
            sendbuf, sendcnt, sendtype, recvbuf, recvcnt, recvtype, root, comm);
    }
}
 ...
 * @param codeptr
 */
static void on_MPIADT_callback_Irecv(void *buf, int count, MPI_Datatype datatype,
                              ...
                              void **tool_data, const void *codeptr)
{
    ...
    if (endpoint == MPIADT_endpoint_begin)
    {
        if (a->callback_Irecv_pre)
            a->callback_Irecv_pre(a->getParallelId(),a->getLocationId(codeptr),
            buf, count, datatype, source, tag, comm, request);
    } else {
        if (a->callback_Irecv_post)
            a->callback_Irecv_post(a->getParallelId(),a->getLocationId(codeptr),
            buf, count, datatype, source, tag, comm, request);
    }
}
 ...
 * @param codeptr
 */
static void on_MPIADT_callback_Isend(const void *buf, int count, MPI_Datatype datatype,
                              ...
                              void **tool_data, void *codeptr)
{
    ...
    if (endpoint == MPIADT_endpoint_begin)
    {
        if (a->callback_Isend_pre)
            a->callback_Isend_pre(a->getParallelId(),a->getLocationId(codeptr),
            buf, count, datatype, dest, tag, comm, request);
    } else {
        if (a->callback_Isend_post)
            a->callback_Isend_post(a->getParallelId(),a->getLocationId(codeptr),
            buf, count, datatype, dest, tag, comm, request);
    }
}
 ...
 * @param codeptr
 */
static void on_MPIADT_callback_Wait(MPI_Request *request, MPI_Status *status,
                             ...
                             const void *codeptr)
{
    ...
    if (endpoint == MPIADT_endpoint_begin)
    {
        if (a->callback_Wait_pre)
            a->callback_Wait_pre(a->getParallelId(),a->getLocationId(codeptr),
            request, status);
    } else {
        if (a->callback_Wait_post)
            a->callback_Wait_post(a->getParallelId(),a->getLocationId(codeptr),
            request, status);
    }
}
 ...
 * @param codeptr
 */
static void on_MPIADT_callback_Waitall(int count, MPI_Request array_of_requests[],
                                ...
                                const void *codeptr)
{
    ...
    if (endpoint == MPIADT_endpoint_begin)
    {
        if (a->callback_Waitall_pre)
            a->callback_Waitall_pre(a->getParallelId(),a->getLocationId(codeptr),
            count, array_of_requests, array_of_statuses);
    } else {
        if (a->callback_Waitall_post)
            a->callback_Waitall_post(a->getParallelId(),a->getLocationId(codeptr),
            count, array_of_requests, array_of_statuses);
    }
}
...
#define register_callback_t(name, type)                            \
    ...
    {                                                              \
        ...
        if (register_fct(MPIADT_##name, (MPIADT_callback_t) f_##name) == \
            MPIADT_set_never)                                        \
