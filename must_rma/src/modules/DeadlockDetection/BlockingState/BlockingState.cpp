...
bool BlockingState::handleNewOp (int rank, BlockingOp* newOp)
{
    ...
    if (!printedTeaser)
    {
        must::cout << "INFORMATION: the exhaustive checkpoint verification test is enabled for this build, you should not use this build for production tests!" << std::endl;
        printedTeaser = true;
    }
    ...
    if (myOrder->isRankOpen(rank))
    {
        ...
        {
            must::cerr << "Internal error in BlockingState, an operation returned PROCESSING_REEXECUTE, which should not happen!" << std::endl;
            assert (0);
        }
    }
    else
...
void BlockingState::printHistoryAsDot (void)
{
    ...
    must::cout << "BlockingState: Printed block history into a file named \"" << MUST_OUTPUT_DIR << "must_block_history.dot\" use DOT to visualize it." << std::endl;
}
#endif
...
//#endif
        ...
        if (enforceAValidSuspensionDecision && hadAValidExploration)
        {
            ...
            if (myOrder->isSuspended())
            {
                must::cerr << "ERROR: Internal MUST error, tried to enforce a valid suspension decision but apparently this was not a valid decission series in the first place." << std::endl;
                assert (0);
            }
        }
    }
