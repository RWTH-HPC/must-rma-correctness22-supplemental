		...
		int source)
{
    ...
    must::cout << "P2PMatch: IGNORED (due to match enforcement) WcRecvUpdate newSource=" << source << " from rank " << myPIdMod->getInfoForId(pId).rank << std::endl;
#endif
        return GTI_ANALYSIS_SUCCESS;
	}
...
#ifdef MUST_MATCH_DEBUG
	must::cout << "P2PMatch: WcRecvUpdate newSource=" << source << " from rank " << myPIdMod->getInfoForId(pId).rank << std::endl;
	printQs ();
#endif
...
#ifdef MUST_MATCH_DEBUG
	        must::cout << "LostMessage: IGNORED (due to enforced matching) WcIrecvUpdate newSource=" << source << " request=" << request << " from rank " << myPIdMod->getInfoForId(pId).rank << std::endl;
#endif
	        return GTI_ANALYSIS_SUCCESS;
	    }
	}
...
#ifdef MUST_MATCH_DEBUG
	must::cout << "LostMessage: WcIrecvUpdate newSource=" << source << " request=" << request << " from rank " << myPIdMod->getInfoForId(pId).rank << std::endl;
	printQs ();
#endif
		...
		MustRequestType request)
{
	...
	if (!warned)
		must::cerr << "P2PMatch: detected a cancel, not supported, outputs may be wrong!" << std::endl;
	warned = true;
...
GTI_ANALYSIS_RETURN P2PMatch::printLostMessages ()
{
	...
	must::cout << "P2PMatch: printed match histories for each comm in files named \"must_match_history_comm_%d.dot\" use DOT to visualize them." << std::endl;
#endif
...
void P2PMatch::printQs (void)
{
	...
	for (qIter = myQs.begin(); qIter != myQs.end(); qIter++)
	{
		...
		for (cIter = qIter->second.begin(); cIter != qIter->second.end(); cIter++)
		{
			...
			for (oIter = t->sends.begin(); oIter != t->sends.end(); oIter++)
			{
				...
				{
				    must::cout << "                ";
					if(*lIter) (*lIter)->print(must::cout);
					must::cout << std::endl;
				}
	...
	{
	    must::cout << "RANK " << rankWcIter->first << ":" << std::endl;
                        ...
                        int *outNumAlternatives)
{
    ...
    if (!unsuspend)
    {
        must::cerr << "Internal Error: tried to enforce a wc-receive source update but failed to remove the suspension with that." << std::endl;
        return false;
    }
                        ...
                        int sourceDest)
{
    ...
    if (sourceDest == myConsts->getAnySource())
    {
         ...
         */
        //must::cout << "Lost message match reduction not supported for wildcards!" << std::endl;
        //assert (0);
        return true;
    }
