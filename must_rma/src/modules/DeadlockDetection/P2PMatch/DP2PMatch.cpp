...
#define NUM_MODS_REQUIRED 8
    ...
    {
        must::cerr << "ERROR: Distributed P2P Matching was mapped on a layer of size > 0 while no intra layer communication was present, as a result P2P matching will not be possible. Either add an intra layer communication, or map the P2P matching onto a layer with a single process." << std::endl;
        assert (0);
    }
}
		...
		int source)
{
	...
	must::cout << "DP2PMatch: WcRecvUpdate newSource=" << source << " from rank " << myPIdMod->getInfoForId(pId).rank << std::endl;
	printQs ();
#endif
		...
		MustRequestType request)
{
	...
	if (!warned)
		must::cerr << "DP2PMatch: detected a cancel, not supported, outputs may be wrong!" << std::endl;
	warned = true;
...
GTI_ANALYSIS_RETURN DP2PMatch::printLostMessages ()
{
	...
	must::cout << "DP2PMatch: printed match histories for each comm in files named \"must_match_history_comm_%d.dot\" use DOT to visualize them." << std::endl;
#endif
...
void DP2PMatch::processQueuedEvents (std::list<int>* ranksToProcess)
{
    ...
    for (iter = ranksToProcess->begin(); iter != ranksToProcess->end(); iter++)
    {
        ...
        while (!mySuspension[rank].queue.empty())
        {
            ...
            {
                must::cerr << "Internal Error when processing an operaiton! " << __FILE__ << ":" << __LINE__ << std::endl;
                assert (0);
                break;
            }
