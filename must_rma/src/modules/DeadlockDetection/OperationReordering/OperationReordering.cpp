...
OperationReordering::~OperationReordering (void)
{
    ...
    for (int i = 0; i < myQueues.size(); i++)
    {
        if (myQueues[i].size())
        {
            must::cout << "Warning: OperationReordering has " << myQueues[i].size() << " operation queued for rank " << i << " during its destruction:" << std::endl;
...
bool OperationReordering::isRankOpen (int rank)
{
    ...
    {
        must::cerr << "ERROR: initialization failed, OperationReordering did not receive the init event." << std::endl;
        assert (0);
        return false;
    }
