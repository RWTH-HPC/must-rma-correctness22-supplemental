...
CollectiveMatch::~CollectiveMatch ()
{
    ...
    {
#ifdef MUST_DEBUG
        must::cout << "CollectiveMatch had collective ops in its matching structure during its destruction!" << std::endl;
#endif
    }
...
bool CollectiveMatch::handleNewOp (int rank, CollectiveOp* newOp)
{
    if (myOrder->isRankOpen(rank))
    {
        ...
        {
            must::cerr << "Internal error in CollectiveMatch, a operation returned PROCESSING_REEXECUTE, which should not happen!" << std::endl;
            assert (0);
        }
    }
    else
