...
// RUN: %must-run %mpiexec-numproc-flag 2 %must-bin-dir/sendDl 2>&1 \
// RUN: | %filecheck --check-prefix 'CHECK-LOCAL' %s

// RUN: %must-run-ddl %mpiexec-numproc-flag 3 %must-bin-dir/DDlsendDl 2>&1 \
// RUN: | %filecheck --check-prefix 'CHECK-DDL' %s

// CHECK-LOCAL: [MUST-REPORT]{{.*The application issued a set of MPI calls that can cause a deadlock!.*MPI_Send}}

// CHECK-DDL: [MUST-REPORT]{{.*The application issued a set of MPI calls that can cause a deadlock!.*MPI_Send}}
