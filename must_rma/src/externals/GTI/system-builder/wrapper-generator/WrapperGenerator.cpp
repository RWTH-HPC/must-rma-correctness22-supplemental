		...
		myAnnotationsEnabled(false)
{
	...
	if (myAnnotationsEnabled) {
		...
		mySourceOut << std::endl << "extern \"C\" int MPI_Pcontrol (int level, ...)" << std::endl
					<< "{" << std::endl
        ...
        bool *pIsWrapDown)
{
	...
	bool isHook = false;
	std::string isWrapAcrossString, isWrapDownString, isCallbackString, isAnnotationString, isHookString;
	...
	if (node.getAttribute("annotation", &isAnnotationString))
	{
	    ...
	    {
	        std::cerr << "|  |-->Error: a call uses the \"annotation\" attribute with an unknown value \"" << isAnnotationString << "\", valid values are \"yes\" and \"no\"." << std::endl;
	        return false;
	    }
	}
...
// ELP: callbacks do not need to call PXXX functions!
	...
	if (myAnnotationsEnabled && isAnnotation) {
		...
		std::transform(enumTypeName.begin()+9, enumTypeName.end(), enumTypeName.begin()+9, ::toupper);
		...
		for (iter = argsnTypes.begin(); iter != argsnTypes.end(); iter++,afterIter++) {
                callArgsString << "arg_buf->" << iter->second << (std::next(iter) == argsnTypes.end() ? "" : ", ");
		...
		myMpiPcontrolString << "\t\tcase " << enumTypeName << ": {" << std::endl
							<< "\t\t\t" << argStructType << "_t" << "* arg_buf = static_cast<" << argStructType << "_t" << "*> (buf->data);" << std::endl
