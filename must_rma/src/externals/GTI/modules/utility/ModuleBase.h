...
#ifdef GTI_THREAD_SAFETY /* Without thread safety we do not differ between TLS \
                            or not */
template <class T> class TLSWrapper {
  T _defaultValue{};
...
#if defined(USE_THREAD_LOCAL) || !defined(GTI_THREAD_SAFETY)
...
#else
  sf::contfree_safe_ptr<std::vector<T *>> _data{};
...
public:
    ...
    if (needs_init) {
      {
        ...
        if (xsafe_data->size() < tid + 1) {
          xsafe_data->resize(tid + 1, nullptr);
        }
        xsafe_data->at(tid) = new T{_defaultValue};
      }
      init();
    }
    return slock_safe_ptr(_data)->at(tid);
  }
  ...
  TLSWrapper(std::function<void(void)> initializer)
...
//          printf("%s: %i instances left\n", __PRETTY_FUNCTION__, iter->second.first->myRefCount);
...
//            printf("%s: %i instances left\n", __PRETTY_FUNCTION__, iter->second.first->myRefCount);
  ...
  ourInstances()
	{
          static TLSWrapper<wrapMap> instances{
