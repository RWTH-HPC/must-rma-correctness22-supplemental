...
namespace gti
{
	...
	void ModuleBase<T, Base, TLS>::initModuleOnce (PNMPI_modHandle_t modHandle)
  {
	...
	PNMPI_modHandle_t ModuleBase<T, Base, TLS>::getWrapperHandle()
	{
          ...
          // Get module handle
#ifdef PNMPI_FIXED
          err = PNMPI_Service_GetModuleByName(
              (ModuleBase<T, Base, TLS>::ourModName)().c_str(), handle);
#else
	    char temp[64];
...
*/
			...
			GTI_Fct_t* pOutFunction)
	{
	    ...
	    if (pOutFunction) *pOutFunction = NULL;
            ...
            static TLSWrapper<PNMPI_Service_descriptor_t> tls_service{[&]() {
              tls_service.setValue(getWrapperService("getFunction", "pp"));
            }};
            auto service = tls_service.getData();

            // Call getFunction to get symbol
            if (((getFunction_t)service->fct)(functionName.c_str(),
                                              pOutFunction) != PNMPI_SUCCESS)
              return GTI_ERROR;
	        ...
	        GTI_Fct_t* pOutFunction)
	{
	    ...
	    if (pOutFunction) *pOutFunction = NULL;
            ...
            static TLSWrapper<PNMPI_Service_descriptor_t> tls_service{[&]() {
              tls_service.setValue(
                  getWrapperService("getAcrossFunction", "pp"));
            }};
            auto service = tls_service.getData();
	        ...
	        GTI_Fct_t* pOutFunction)
    {
	    ...
	    if (pOutFunction) *pOutFunction = NULL;
            static TLSWrapper<PNMPI_Service_descriptor_t> tls_service{[&]() {
              tls_service.setValue(
                  getWrapperService("getBroadcastFunction", "pp"));
            }};
            auto service = tls_service.getData();
	...
	GTI_RETURN ModuleBase<T, Base, TLS>::getSetNextEventStridedFunction (GTI_Fct_t* pOutFunction)
	{
	    ...
	    if (pOutFunction) *pOutFunction = NULL;
            static TLSWrapper<PNMPI_Service_descriptor_t> tls_service{[&]() {
              tls_service.setValue(
                  getWrapperService("getNextEventStrided", "p"));
            }};
            auto service = tls_service.getData();

            //Call getFunction to get symbol
            if (((getFunctionNoName_t)service->fct)(pOutFunction) !=
                PNMPI_SUCCESS)
              return GTI_ERROR;
